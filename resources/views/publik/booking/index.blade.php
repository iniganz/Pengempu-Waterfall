                    ...
                    <form method="POST" action="{{ route('booking.store', $product) }}" id="booking-form">
                        ...
                        <div class="d-flex flex-wrap align-items-start gap-3">
                            <div style="flex:1 1 260px;">
                                <div class="fw-semibold">Person @error('qty') <span class="text-danger">*</span> @enderror</div>
                                <div class="text-muted small">IDR {{ number_format($price, 0, ',', '.') }} / Ticket</div>
                                <div class="d-flex align-items-center gap-2 mt-2">
                                    <button type="button" class="btn btn-outline-secondary btn-sm" id="qty-dec">−</button>
                                    <input id="qty-input" name="qty" type="number" min="1" value="{{ old('qty', 1) }}" class="form-control form-control-sm @error('qty') is-invalid @enderror" style="max-width:80px;">
                                    <button type="button" class="btn btn-outline-secondary btn-sm" id="qty-inc">+</button>
                                </div>
                                @error('qty')
                            ...
                            <div style="flex:1 1 260px;">
                                <label class="form-label fw-semibold">Reserve Date @error('reserve_date') <span class="text-danger">*</span> @enderror</label>
                                <input type="date" id="reserve-date" name="reserve_date" class="form-control @error('reserve_date') is-invalid @enderror" value="{{ old('reserve_date', $product->date ?? now()->toDateString()) }}">
                                @error('reserve_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="text-end ms-auto" style="min-width: 180px;">
                                ...
                                <div class="fs-4 fw-bold text-success" id="total-price">IDR {{ number_format($price, 0, ',', '.') }}</div>
                                <button type="submit" class="btn btn-success w-100 mt-2" id="btn-continue">Continue</button>
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label">Nama @error('name') <span class="text-danger">*</span> @enderror</label>
                                <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Nama lengkap" value="{{ old('name') }}">
                                @error('name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4">
                                <label class="form-label">No HP @error('phone') <span class="text-danger">*</span> @enderror</label>
                                <input type="tel" id="phone" name="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="08xxxx" value="{{ old('phone') }}">
                                @error('phone')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-sm mb-0">
                        <tr>
                            ...
                            <td class="small" id="reserve-date-display">{{ $product->date ?? now()->toDateString() }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="small">Jl. Seribupati, Cau Belayu, Kec. Marga, Kabupaten Tabanan, Bali 82181</td>
                        </tr>
                        <tr>
                    ...
                    <div id="identity-summary" class="mt-3" style="display:none;">
                        ...
                        <ul class="mb-0 small" id="identity-list"></ul>
                    </div>
                </div>
            </div>
        </div>
...
<div id="confirm-overlay" class="confirm-overlay">
    <div class="confirm-card">
        ...
        <ul class="list-unstyled mb-3 small" id="confirm-list"></ul>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (form) {
            form.addEventListener('submit', function (e) {
                const qtyOk = qtyInput && clampQty(qtyInput.value) >= 1;
                const dateOk = reserveInput && reserveInput.value;
                const allFilled = updateIdentitySummary();

                if (!qtyOk || !dateOk || !allFilled) {
                    e.preventDefault();
                    alert('Lengkapi qty, tanggal, nama, email, dan no HP terlebih dahulu.');
                    return;
                }

                e.preventDefault();
                if (overlay && overlayList && nameInput && emailInput && phoneInput && reserveInput && totalEl) {
        ...
        if (overlayYes) {
            overlayYes.addEventListener('click', function () {
                if (overlay) overlay.style.display = 'none';
                if (form) form.submit();
            });
        }
